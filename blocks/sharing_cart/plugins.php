...
class sharing_cart_plugins
{
	...
	{
		if ($record = get_record('sharing_cart_plugins', 'plugin', $plugin, 'sc_user', $user)) {
	...
	{
		if (record_exists('sharing_cart_plugins', 'plugin', $plugin, 'sc_user', $user)) {
			return set_field('sharing_cart_plugins', 'data', addslashes(serialize($data)),
			                 'plugin', $plugin, 'sc_user', $user);
		} else {
			$record         = new stdClass;
