...
class block_flickr_image_carousel extends block_base {
    ...
    function specialization() {
		...
		if (isset($this->config->clearcache)) {
			$this->f->enableCache("fs", $CFG->dirroot.'/blocks/flickr_image_carousel/img-cache/', -1);			
			unset($this->config->clearcache);
			$this->instance_config_save($this->config);
			
		} else {
        	$this->f->enableCache("fs", $CFG->dirroot.'/blocks/flickr_image_carousel/img-cache/', 604800); // cache for 7 days
		}
    ...
    private function build_film_strip($photos, $userno) {
        ...
        $photoslist = array(
			'<script type="text/javascript" src="http://yui.yahooapis.com/combo?2.8.1/build/carousel/carousel-min.js"></script>  '.
            '<link rel="stylesheet" type="text/css" href="http://yui.yahooapis.com/combo?2.8.1/build/carousel/assets/skins/sam/carousel.css">'.
