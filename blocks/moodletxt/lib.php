...
' . $listString . '
    ...
    */
    // above function re-writeen so that phone number can be taken from a user_info field
    // MH Oct 2010
    function moodletxt_get_mobile_number(&$user) {

        global $NATIONAL_PREFIX, $CFG;

        
        if ($dataPhone = get_record_sql("SELECT * FROM {$CFG->prefix}user_info_data WHERE userid={$user->id} AND fieldid=41")) {
          $userphone = $dataPhone->data; 
        } else {
          return '';
        }
    ...
    function moodletxt_update_outbound_stats($txttoolsaccount, $userid, $numbersent) {
        ...
        // $getTodaysStats = get_record('block_mtxt_stats', 'txttoolsaccount', $txttoolsaccount, 'userid', $userid, 'date_entered', date('Y-m-d') . '00:00:00'); // MH this no work in Oracle! 
        $getTodaysStats = get_record('block_mtxt_stats', 'txttoolsaccount', $txttoolsaccount, 'userid', $userid, 'date_entered', date('Y-m-d')); 
        ...
        if (is_object($getTodaysStats)) {
            ...
            //$todaysStats->date_entered = date('Y-m-d') . ' 00:00:00'; // MH this no work in Oracle!
            $todaysStats->date_entered = date('Y-m-d');
