<?php // $Id: questions_form.php,v 1.21.2.3 2008/10/04 12:57:47 joseph_rezeau Exp $
/**
* print the form to add or edit a questionnaire-instance
*
* @version $Id: questions_form.php,v 1.21.2.3 2008/10/04 12:57:47 joseph_rezeau Exp $
...
class questionnaire_questions_form extends moodleform {
    ...
    function definition() {
        ...
        foreach ($qtypes as $key => $qtype) {
            if ($qtype!='File') {
                $qtypes_lang[$key] = questionnaire_get_type($key); 
            }
        }
        $qtypes = $qtypes_lang;
        natsort($qtypes);
        ...
        // Add preview button        
        //$mform->addElement('html', "<a target='_blank' href='".$CFG->wwwroot.htmlspecialchars('/mod/questionnaire/preview.php?id='.$questionnaire->cm->id)."'>".get_string('preview_label', 'questionnaire')."</a>");
    }
...
class questionnaire_edit_question_form extends moodleform {
    ...
    function definition() {
        ...
        if ($question->type_id != QUESSECTIONTEXT && $question->type_id != '') {
            ...
            $mform->addElement('text', 'name', get_string('optionalname', 'questionnaire'), array('size'=>'25', 'maxlength'=>'25'));
        ...
        if ($question->type_id == QUESYESNO || $question->type_id == QUESDROP || $question->type_id == QUESDATE ||
            $question->type_id == QUESSECTIONTEXT || $question->type_id == QUESFILE) {
        ...
        /// Precision field:
        if ($question->type_id == QUESYESNO || $question->type_id == QUESDROP || $question->type_id == QUESDATE ||
            $question->type_id == QUESSECTIONTEXT || $question->type_id == QUESRADIO || $question->type_id == QUESFILE) {
