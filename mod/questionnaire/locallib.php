<?php // $Id: locallib.php,v 1.49.2.50 2009/03/10 15:03:03 joseph_rezeau Exp $
...
//for 1.7        $this->context = get_context_instance(CONTEXT_MODULE, $cm->id);
    ...
    function view() {
        ...
        if ( !empty($this->survey->theme) ) {
            ...
            if($this->survey->theme=='agora.css' && file_exists($CFG->dataroot.'/1/theme/styles_enquesta.css')){
        		$currentcss.='<link rel="stylesheet" type="text/css" href="'.
                    $CFG->wwwroot.'/file.php/1/theme/styles_enquesta.css" />';
             }
    ...
    function print_survey_start($message, $section, $num_sections, $has_required, $rid='') {
        ...
        if ($resp) {
            ...
            // JR DEV comment following line out if you do NOT want time submitted displayed in Anonymous surveys
                $timesubmitted = '&nbsp;'.get_string('submitted', 'questionnaire').'&nbsp;<span class="submitted-data">'.userdate($resp->submitted).'</span>';
            }
        }
        if ($ruser) {
            echo ('<div id="user-info">');
            echo ('<span class="user">'.get_string('respondent', 'questionnaire').': <span class="user-data">'.$ruser.'</span></span>');
            ...
            echo ($groupname);
            echo ('<span class="submitted">'.$timesubmitted.'</span>');
            echo ('</div>');
        }
...
//            $title = userdate($ridssub[$curr_pos]);
    ...
    ?>
    <h3><?php echo($this->survey->title); ?></h3>
    <?php
        if ($this->survey->subtitle) {
...
/* {{{ proto array survey_generate_csv(int survey_id)
    ...
    */
    function generate_csv($rid='', $userid='', $choicecodes=1, $choicetext=0) {
        ...
        $output = array();
        // CMDL-1653 Moodle Questionnaire report enhancement (REQ0029298)
