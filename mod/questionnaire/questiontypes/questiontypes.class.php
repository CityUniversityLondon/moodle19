<?php // $Id: questiontypes.class.php,v 1.31.2.31 2009/03/14 10:29:04 joseph_rezeau Exp $
...
/// appropriate function based on its table.
    ...
    function get_response_date_results($rids = false) {
        ...
        $sql = 'SELECT '. sql_order_by_text('response', $numchars=4000) .'AS response, COUNT(response_id) AS num '.
               'FROM '.$CFG->prefix.'questionnaire_'.$this->response_table.' '.
               'WHERE question_id='.$this->id.$ridstr.' '.
               'GROUP BY '. sql_order_by_text('response', $numchars=4000);
    ...
    function get_response_rank_results($rids=false) {
        ...
        if($this->type_id  == 8) { //Rank
         // JR there can't be an !other field in rating questions ???
            ...
            $sql = 'SELECT ' . sql_order_by_text('c.content', $numchars=4000). 'AS content, AVG(A.rank+1) AS average '.
                   ...
                   'GROUP BY ' .sql_order_by_text('c.content', $numchars=4000);
            return get_records_sql($sql);
        } else {
            ...
            while(list($content) = each($this->counts)) {
                ...
                // eliminate potential named degrees on Likert scale
                 if (!ereg("^[0-9]{1,3}=", $content)) {
                    ...
                    if (!$isna) {
                        if ($osgood) {
                            $table->data[] = array('<div align="right">'.$content.'</div>', $out, '<div align="left"><strong>'.$contentright.'</strong></div>', sprintf('%.1f', $avg));
                        } else {
                            $table->data[] = array($content, $out, sprintf('%.1f', $avg));
                        }
                    } else {
                        if ($avg) {
                            $avg = sprintf('%.1f', $avg);
