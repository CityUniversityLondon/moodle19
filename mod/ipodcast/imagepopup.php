...
/******************************************************************************\
 *
 * Filename:    imagepopup.php
 *
 *		This file pops up a file window to select attachment
 *
 *
 * History:     10/23/06 Tom Dolsky     - Created imagepopup.php file
    ...
    switch ($action) {
        ...
        case "upload":
            ...
            if (!empty($save) and confirm_sesskey()) {
                ...
                displaydir($ipodcast_course, $wdir);
            } else {
                ...
                echo "<form enctype=\"multipart/form-data\" method=\"post\" action=\"imagepopup.php\">\n";
                ...
                echo "<form action=\"imagepopup.php\" method=\"get\">\n";
                echo ' <input type="hidden" name="choose" value="'.$choose.'">';
                echo " <input type=\"hidden\" name=\"id\" value=\"$id\" />\n";
                echo " <input type=\"hidden\" name=\"wdir\" value=\"$wdir\" />\n";
            ...
            if (!empty($confirm) and confirm_sesskey()) {
                ...
                if (setfilelist($_POST)) {
                    ...
                    notice_yesno (get_string("deletecheckfiles"), 
                                "imagepopup.php?id=$id&amp;wdir=$wdir&amp;action=delete&amp;confirm=1&amp;sesskey=$USER->sesskey&amp;choose=$choose",
                                "imagepopup.php?id=$id&amp;wdir=$wdir&amp;action=cancel&amp;choose=$choose");
                } else {
                    displaydir($ipodcast_course, $wdir);
                }
                html_footer();
        ...
        case "rename":
            if (!empty($name) and confirm_sesskey()) {
                ...
                echo "<table><tr><td>";
                echo "<form action=\"imagepopup.php\" method=\"post\" name=\"form\">";
                echo ' <input type="hidden" name="choose" value="'.$choose.'">';
                echo " <input type=\"hidden\" name=\"id\" value=\"$id\" />";
                echo " <input type=\"hidden\" name=\"wdir\" value=\"$wdir\" />";
        ...
        case "select":
			...
			if (!(file_exists($basedir.$wdir."/".$name))) {			
				...
				if($wdir == "/") {
					$ipodcast_course->image = "/" .$name;
				} else {
					$ipodcast_course->image = $wdir."/".$name;			
				}
				
				$ipodcast_course->instance = $ipodcast_course->id;
				if(ipodcast_update_course_instance(addslashes_object($ipodcast_course))) {
...
function displaydir ($ipodcast_course, $wdir) {
//  $wdir == / or /a or /a/b/c/d  etc
	...
	$selectedfilename = $ipodcast_course->image;
    ...
    echo "<form action=\"imagepopup.php\" method=\"post\" name=\"dirform\">\n";
    echo '<input type="hidden" name="choose" value="'.$choose.'">';
...
//   }
    ...
    if (!empty($dirlist)) {
        ...
        foreach ($dirlist as $dir) {
            ...
            if ($dir == '..') {
                ...
                print_cell();
                print_cell('left', '<a href="imagepopup.php?id='.$id.'&amp;wdir='.$fileurl.'&amp;choose='.$choose.'"><img src="'.$CFG->pixpath.'/f/parent.gif" height="16" width="16" alt="'.get_string('parentfolder').'" /></a> <a href="imagepopup.php?id='.$id.'&amp;wdir='.$fileurl.'&amp;choose='.$choose.'">'.get_string('parentfolder').'</a>', 'name');
                print_cell();
                print_cell();
                print_cell();
                ...
                print_cell("left", "<a href=\"imagepopup.php?id=$id&amp;wdir=$fileurl&amp;choose=$choose\"><img src=\"$CFG->pixpath/f/folder.gif\" height=\"16\" width=\"16\" border=\"0\" alt=\"Folder\" /></a> <a href=\"imagepopup.php?id=$id&amp;wdir=$fileurl&amp;choose=$choose\">".htmlspecialchars($dir)."</a>", 'name');
    ...
    if (!empty($filelist)) {
		...
		$tohash = "$ipodcast_course->course:$USER->id:0:$filetocompare:$ipodcast_course->authkey";
		$hash = md5($tohash);

      	if ($CFG->slasharguments) {
			$ffurl = "/mod/ipodcast/file.jpg/$hash/$ipodcast_course->course/$USER->id/ipodcast/0";
      	} else {
			$ffurl = "/mod/ipodcast/file.jpg?file=/$hash/$ipodcast_course->course/$USER->id/ipodcast/0";
      	}
		...
		$newffurl = cleardoubleslashes($ffurl . $filetocompare);
            ...
            if ($icon == "text.gif" || $icon == "html.gif") {
                $edittext .= "<a href=\"imagepopup.php?id=$id&amp;wdir=$wdir&amp;file=$fileurl&amp;action=edit&amp;choose=$choose\">$stredit</a>";
            } else if ($icon == "zip.gif") {
                $edittext .= "<a href=\"imagepopup.php?id=$id&amp;wdir=$wdir&amp;file=$fileurl&amp;action=unzip&amp;sesskey=$USER->sesskey&amp;choose=$choose\">$strunzip</a>&nbsp;";
                $edittext .= "<a href=\"imagepopup.php?id=$id&amp;wdir=$wdir&amp;file=$fileurl&amp;action=listzip&amp;sesskey=$USER->sesskey&amp;choose=$choose\">$strlist</a> ";
                if (!empty($CFG->backup_version) and $isteacheredit) {
                    $edittext .= "<a href=\"imagepopup.php?id=$id&amp;wdir=$wdir&amp;file=$filesafe&amp;action=restore&amp;sesskey=$USER->sesskey&amp;choose=$choose\">$strrestore</a> ";
                }
            }
		...
		//echo "filetocompare = " . $filetocompare . "<br>\n" ;
			...
			if(strcmp($selectedfilename,$filetocompare) == 0) {
				...
				print_cell("right", "$edittext <a href=\"imagepopup.php?id=$id&amp;wdir=$wdir&amp;name=$filesafe&amp;action=select&amp;choose=$choose\">$strselect</a>", 'commands');
			}
    		}
            echo "</tr>";
        }
