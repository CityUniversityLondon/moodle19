...
function bookmarks_add_bookmark($name, $description, $url, $tags, $bookmarksid, $userid=null){
	...
	if(preg_match($reg,$url)){
		...
		if (!empty($link)){
			$exists = count_records("bookmarks_items", 'bookmarksid', $bookmarksid, 'linkid', $link->id);
			if ($exists != 0){
	...
	return tag_set('bookmarks_items',$itemid, explode(',',$tags));
}
...
function bookmarks_update_bookmark($itemid, $name, $description, $url, $tags, $userid=null) {
	...
	return tag_set('bookmarks_items',$item->id, explode(',',$tags));
}
...
function bookmarks_delete_item($item){
	...
	if (!empty($tags)){
		foreach ($tags as $tag){
			tag_delete_instance('bookmarks_items',$item->id, $tag->id);
		}
	}
...
function bookmarks_untag($itemid, $tagname){
	...
	return tag_delete_instance('bookmarks_items',$itemid, $tag->id);
}
...
function bookmarks_get_item_tags($id){
	return tag_get_tags('bookmarks_items',$id);
...
function bookmarks_search_by($criteria, $id){
    ...
    // AD changed: $query = 'SELECT DISTINCT i.*
	$query = 'SELECT DISTINCT i.id, i.name, i.userid, i.linkid,i.bookmarksid, to_char(i.description)
	...
	if (!empty($queries['text'])){
		foreach ($queries['text'] as $text){
			$query .= ' AND (';
            $query .= ' ' . sql_olike('i.name', $text) . 'OR';
            $query .= ' ' . sql_olike('i.description', $text);
			$query .= ')'; 
		}
	...
	if (!empty($queries['tag'])){
		foreach ($queries['tag'] as $tag){
			$query .= ' AND '; 
			$query .= "t.name = '" . strtolower($tag) . "'";
		}
	}
	if (!empty($queries['inurl'])){
		foreach ($queries['inurl'] as $url){
			$query .= ' AND ';
            $query .= sql_olike('l.url', $url);
		}
	}
...
function bookmarks_get_activity_tags($bookmarksid){
	...
	$query = 'SELECT t.id, t.name, t.rawname, count(*) as count 
	...
	$query = 'SELECT t.id, t.name, t.rawname, count(*) as count 
				FROM '.$CFG->prefix.'tag t LEFT OUTER JOIN '.$CFG->prefix.'tag_instance ins 
					...
					LEFT OUTER JOIN '.$CFG->prefix.'bookmarks_items it ON 
						ins.itemid = it.id
				WHERE it.bookmarksid = '.$bookmarksid.' AND 
					ins.itemtype = \'bookmarks_items\' AND
					it.userid = '. $userid. ' 				
				GROUP BY t.id, t.name, t.rawname';
