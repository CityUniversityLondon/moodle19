...
function scheduler_get_possible_attendees($cmid,$groups=''){
    $context = get_context_instance(CONTEXT_MODULE, $cmid);
    $attendees = get_users_by_capability($context, 'mod/scheduler:appoint', 'u.id, firstname, lastname, email, picture', 'lastname, firstname', '', '', $groups, '', false, false, false);
    return $attendees;
}
...
function scheduler_get_slot_location($slotid){
    ...
    $location = get_record('scheduler_slots', 'id', $slotid);
    if( !$location->appointmentlocation) {
        $slotloc =  '<span style="text-align: center;"> - </span>';
    } else {
        $slotloc = $location->appointmentlocation;
    }
    return $slotloc;
}
// end CMDl-1667

// CMDL-1733 Scheduler Export Errors (INC0041083)
