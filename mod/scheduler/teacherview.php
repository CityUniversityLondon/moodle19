...
if ($action == 'schedule') {    
	...
	// end CMDL-1739
        ...
        /// set an advice message
        foreach($appointments as $appointment) {
            if ($conflictstudent = scheduler_get_conflicts($scheduler->id,
                $form->starttime, $form->starttime + $form->duration * 60, 0,
                $studentid, SCHEDULER_ALL, false)) {
                $student = get_record('user', 'id', $appointment->studentid);
                $studentname = fullname($student, true);
                unset($erroritem);
                $erroritem->message = get_string('studentbusy', 'scheduler', $studentname);
...
if ($action == 'schedulegroup') { 
    if($subaction == 'dochooseslot'){        
        ...
        foreach($appointments as $appointment){
            if ($conflictstudent = scheduler_get_conflicts($scheduler->id,
                $form->starttime, $form->starttime + $form->duration * 60, 0,
                $appointment->studentid, SCHEDULER_ALL, false)) {
                $student = get_record('user', 'id', $appointment->studentid);
                $studentname = fullname($student, true);
                unset($erroritem);
                $erroritem->message = get_string('studentbusy', 'scheduler', $studentname);
...
if ($slots){
    ...
    if ($page == 'myappointments'){
        ...
        // CMDL-1667 Scheduler tool doesn't show the location field attribute to students (INC0026350)
        ...
        $table->head  = array ('', $strdate, $strstart, $strend, $strlocation, $strstudents, $straction);
        $table->align = array ('CENTER', 'LEFT', 'LEFT', 'CENTER', 'CENTER', 'CENTER', 'CENTER', 'LEFT', 'CENTER');
        // end CMDl-1667
        ...
        $table->head  = array ('', $strdate, $strstart, $strend, $strlocation, $strstudents, format_string($scheduler->staffrolename), $straction);
        $table->align = array ('CENTER', 'LEFT', 'LEFT', 'CENTER', 'CENTER', 'CENTER', 'CENTER', 'LEFT', 'LEFT', 'CENTER');
        // end CMDl-1667
    ...
    foreach($slots as $slot) {
        ...
        // CMDL-1667 Scheduler tool doesn't show the location field attribute to students (INC0026350)
        $loc = scheduler_get_slot_location($slot->id);
        // end CMDl-1667
        ...
        if($page == 'myappointments'){
            ...
            $table->data[] = array ($selectcheck, ($offsetdate == $offsetdatemem) ? '' : $offsetdate, $offsettime, $endtime, $loc, implode("\n",$studentArray), $actions);
            // end CMDl-1667
...
/// print table of outstanding appointer (groups) 
    ...
    if (empty($groups)){
        ...
        foreach($groups as $group){
            $members = get_group_users($group->id, 'u.lastname', '', 'u.id, u.lastname, firstname, email, picture');
