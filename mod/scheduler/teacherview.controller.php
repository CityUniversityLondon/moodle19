...
*/
    case 'doaddupdateslot':{
        ...
        // CMDL-1738 Scheduler: can't delete last user (INC0041397)
...
/************************************ Saving a session with slots *************************************/
    case 'doaddsession':{
        ...
        $startfrom += dst_offset_on( $startfrom, get_user_timezone());
        $noslotsallowed = true;
        for ($d = 0; $d <= $fordays; $d ++){
        ...
        // Date is stored in the databse as Unix timestamp
        // Date displayed is adjusted according to user timezone settings
        // Therefore we want the calculations to allow for DST
        // We add the offset to adjust $startfrom to the GMT equivilent
        // of the display date
        // Example: GMT: 11.00 BST: 12:00 DST Offset: 1
        // We want all our slots to start from BST: 12:00
        // We add the offset to get GMT: 12:00
        // Then for each day we remove the offset
        // So during DST we deduct 1 to get GMT: 11:00 BST: 12:00
        // And outside DST we deduct 0 to get GMT: 12:00 BST: 12:00
        // It has taken me two days to get my head around this
        ...
        for ($d = 0; $d <= $fordays; $d ++){
            ...
            if ((($dayofweek == 'Monday') && ($data->monday == 1)) ||
                ...
                // Here is where we deduct the offset so that our stored unix
                // timestamp is correct
                $slot->starttime -= dst_offset_on($slot->starttime, get_user_timezone());
                $data->timestart = $startfrom + ($d * DAYSECS);
                $data->timestart -= dst_offset_on($data->timestart, get_user_timezone());
                $data->timeend = make_timestamp(date('Y',$data->timestart), date('m',$data->timestart), date('d',$data->timestart), $data->endhour, $data->endminute, 0, get_user_timezone(), true);
                        ...
                        if (!$data->forcewhenoverlap){
                            ...
                            foreach ($conflicts as $aConflict){
                                $sql = "
                                   ...
                                   FROM
                                      {$CFG->prefix}course c,
                                      {$CFG->prefix}scheduler s,
                                      {$CFG->prefix}scheduler_slots sl
                                   WHERE
                                      s.course = c.id AND
...
/************************************ Deleting unused slots (all teachers) ************************************/
    case 'deleteallunused': {
       ...
       if (has_capability('mod/scheduler:manageallappointments', $context)){
			...
			// CMDL-1739 Scheduler: Sessions disappear (INC0041397)
            if ($unappointed) {
			// end CMDL-1739
