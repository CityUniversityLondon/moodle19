...
if ($action == 'savechoice') {
    ...
    // CMDL-1737 Scheduler: multiple signup (INC0041397)
    if ($updating = count_records('scheduler_appointment', 'slotid', $slot->id, 'studentid', $USER->id)){
       print_simple_box_start('center');
       echo '<h2 style="color:red;">'.get_string('slot_already_selected', 'scheduler').'</h2>';
       print_simple_box_end();
       print_continue("{$CFG->wwwroot}/mod/scheduler/view.php?id={$cm->id}");
       print_footer($course);
       exit();
    ...
    foreach($oldslotowners as $studentid) {
        if ($conflictstudent = scheduler_get_conflicts($slot->schedulerid,
            $slot->starttime, $slot->starttime + $slot->duration * 60, 0,
            $studentid, SCHEDULER_ALL, false)) {
            $student = get_record('user', 'id', $studentid);
            $studentname = fullname($student, true);
            unset($erroritem);
            $message .= get_string('studentbusy', 'scheduler', $studentname);
        }
    }
...
if ($action == 'disengage') {
    ...
    // CMDL-1643 Allow student to delete/change specific appointments rather than all appointments
    $where = "studentid = $USER->id AND attended = 0 AND " .
             "EXISTS(SELECT 1 FROM {$CFG->prefix}scheduler_slots sl WHERE sl.id = slotid AND sl.schedulerid = $scheduler->id )";
    //$appointments = get_records_select('scheduler_appointment', "studentid = $USER->id AND attended = 0");
    $appointments = get_records_select('scheduler_appointment', $where);
    // end CMDL-1643
