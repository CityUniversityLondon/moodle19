...
/// prepare appointable slot table
        ...
        foreach($studentSlots as $key => $aSlot){
            ...
            if ($aSlot->appointedbyme and !$aSlot->attended){
                ...
                $table->data[] = array ("<b>$startdatestr</b>", "<b>$starttime</b>", "<b>$endtime</b>", "<b>$loc</b>", $radio, "<b>"."<a href=\"../../user/view.php?id={$aSlot->teacherid}&amp;course=$scheduler->course\">".fullname(get_record('user', 'id', $aSlot->teacherid)).'</a></b>','<b>'.$aSlot->groupsession.'</b>');
                // end CMDl-1667
                ...
                $table->data[] = array ($startdatestr, $starttimestr, $endtimestr, $loc, $radio, "<a href=\"../../user/view.php?id={$aSlot->teacherid}&amp;course={$scheduler->course}\">".fullname(get_record('user', 'id', $aSlot->teacherid)).'</a>', $aSlot->groupsession);
            }
            $previoustime = $starttime;
            $previousendtime = $endtime;
            $previousdate = $startdate;
