...
if ((isset($_POST['option']) and $_POST['option'] == translate('Yes')) or $howeverInstall) {
    ...
    if (full_copy('./install/filter/wiris', '../filter/wiris/')) {
        ...
        if ($response == ALL_WELL) {
            ...
            if (file_exists('./install/filter/wiris/wrs_config.php')) {
                require_once('./install/filter/wiris/wrs_config.php');
            } else {
            ...
            while ($itemReaded = readdir($dirList)) {
                if ($itemReaded != '.' and $itemReaded != '..' and substr($itemReaded, strlen($itemReaded) - 4, 4) == '.php') {
                    require_once('./install/servers/' . $itemReaded);
            ...
            echo translate('Plugin WIRIS is already installed. Click this button to update or reinstall it:'), '<br /><br />';
            echo '<input type="submit" name="restore" value="', addSlashesOnDoubleQuotes(translate('Update or reinstall')) , '"><br /><br />';
			
			echo translate('If you want to uninstall Plugin WIRIS click') . ' <a href="http://www.wiris.com/en/quizzes/docs/install/uninstall">' . translate('here') . '</a>.<br/><br/>';
