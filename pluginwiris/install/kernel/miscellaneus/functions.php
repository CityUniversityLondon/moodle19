...
function errorMessage($tag = '') {
    $toReturn = translate('If you want to install or update Plugin WIRIS manually, see') . ' <a href="http://www.wiris.com/es/plugins/docs/moodle/install/install-manually-version-2-3#' . $tag . '">' . translate('our manual') . '</a>.<br /><br />';
...
function downloadEditor() {
    if (isset($_POST['proxy']) and isset($_POST['proxy_host']) and isset($_POST['proxy_port'])) {
        ...
        if (($content = file_get_contents('http://www.wiris.com/download/editor/download?file=wiriseditor.zip', false, $cxContext)) !== false) {
            return file_put_contents('./install/filter/wiris/editor/jar/pluginwiris.zip', $content);
        }        
        return false;
    }    
    if (($content = wrs_http_get_contents('http://www.wiris.com/download/editor/download?file=wiriseditor.zip')) !== false) {
        return wrs_file_put_contents('./install/filter/wiris/editor/jar/pluginwiris.zip', $content);
    }
    return false;
}

function extractEditor() {
    $handle = new PclZip('./install/filter/wiris/editor/jar/pluginwiris.zip');
    if ($handle->extract(PCLZIP_OPT_PATH, "./install/filter/wiris/editor/jar/") === false) {
        return false;
    }
    return true;
}
...
function parseWeblib() {
    if (($content = file_get_contents('../lib/weblib.php')) !== false) {
        ...
        if (isset($_POST['proxy']) and isset($_POST['proxy_host']) and isset($_POST['proxy_port'])) {
            ...
            $toEval = file_get_contents('http://www.wiris.com/download/moodle/versions.php?version=' . urlencode($release) . '&plugin=new', false, $cxContext);
        } else {
            $toEval = wrs_http_get_contents('http://www.wiris.com/download/moodle/versions.php?version=' . urlencode($release). '&plugin=new');
        }
...
function parseConfig() {
    global $CFG;
    if (file_exists('../filter/wiris/wrs_config.php')) {
    ...
    if (file_put_contents('../filter/wiris/wrs_config.php', $config) === false) {
        return NO_WRITE_PERMISION;
    }
    return ALL_WELL;
}
...
function wrs_file_put_contents($file, $content) {
    if (($handle = fopen('./install/filter/wiris/editor/jar/pluginwiris.zip', 'wb')) !== false) {
