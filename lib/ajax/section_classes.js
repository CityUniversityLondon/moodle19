...
resource_class.prototype.delete_button = function() {
    ...
    // CMDL-1448 and CMDL-1561
    var activity = this.getEl().attributes['class'].value.substring(9);    

    if (activity != 'label') {
        var span = YAHOO.util.Selector.query('a span', this.getEl(), true);
        if(span) {
            var assignmentname = span.innerHTML.split("<", 1)[0];
        } else {
            var assignmentname = YAHOO.util.Selector.query('a', this.getEl(), true).innerHTML;
        }
    } else {
        var assignmentname = '';
    }

    var alerttext = main.getString('deletecheckfull', activity+" "+assignmentname);

    if (activity != 'resource' && activity != 'label') {
        alerttext += "\n\n"+main.getString('deleteasscheckfull');
    }

    if (!confirm(alerttext)) {
    // end CMDL-1448 and CMDL-1561
        return false;
    }
