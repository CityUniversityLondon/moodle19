 ...
 *
 * The implementation of this function is a bit complex, so there are unit
...
function wrsqz_openServlet($data) {
    ...
    stream_context_set_option($context, 'http', 'header', 'Content-Type: application/x-www-form-urlencoded; charset=UTF-8');
    stream_context_set_option($context, 'http', 'content', $postdata);

		if (isset($CFG->wirisproxy) and $CFG->wirisproxy) {
      stream_context_set_option($context, 'http','proxy', 'tcp://' . $CFG->wirisproxy_host . ':' . $CFG->wirisproxy_port);
...
function wrsqz_isEmptySession($session) {
	if (strpos($session,"<mo") !== false) return false;
	if (strpos($session,"<mi") !== false) return false;
	if (strpos($session,"<mn") !== false) return false;
	if (strpos($session,"<csymbol") !== false) return false;
	return true;	
}
function wrsqz_isEmptyMathML($mathml){
  if (strpos($mathml, "<mo") !== false) return false;
  if (strpos($mathml, "<mi") !== false) return false;
  if (strpos($mathml, "<mn") !== false) return false;
  if (strpos($mathml, "<ms") !== false) return false;
  if (strpos($mathml, "<mfrac") !== false) return false;
  if (strpos($mathml, "<mroot") !== false) return false;
  if (strpos($mathml, "<mtext") !== false) return false;
  return true;
}
...
/**
 * @return the default grammar url: http://www.wiris.net/demo/quizzes/grammar/Expression
 * **/
