...
function wrsqz_presave_question($questionType, $dbType, &$question, &$form, $course){
  ...
  if(!empty($form->generalfeedback)){
    $form->generalfeedback = wrsqz_prepareVariablesInsideFormulas($form->generalfeedback);
  }
  if($questionType == 'shortanswer' || $questionType == 'multichoice'){
    foreach ($form->answer as $key => $dataanswer) {
      if(!empty($form->feedback[$key])){
        $form->feedback[$key] = wrsqz_prepareVariablesInsideFormulas($form->feedback[$key]);
      }
    }
  }else if($questionType == 'essay'){
    if(!empty($form->feedback)){
      $form->feedback = wrsqz_prepareVariablesInsideFormulas($form->feedback);
    }
  }else if($questionType == 'truefalse'){
    if(!empty($form->feedbacktrue)){
      $form->feedbacktrue = wrsqz_prepareVariablesInsideFormulas($form->feedbacktrue);
    }
    if(!empty($form->feedbackfalse)){
      $form->feedbackfalse= wrsqz_prepareVariablesInsideFormulas($form->feedbackfalse);
    }
  }
}
...
function wrsqz_response_summary($questionType, $dbType, $question, $state, $length=80){
    if($questionType == 'match'){
       ...
       $summary = print_table($table, true);
        ...
        if(isset($state->responses[''])){
            $response = stripslashes($state->responses['']);
            mb_parse_str($question->options->wiris->eqoption, $eqoptionArray);
            if(isset($eqoptionArray['editor']) && $eqoptionArray['editor'] == 'true'){
                if(!wrsqz_isEmptyMathML($response)){
                  $summary = format_text(wrsqz_mathmlEncode($response),FORMAT_HTML);
                }else{
                  $summary = '';
                }
            }else{
